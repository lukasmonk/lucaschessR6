...
from Code.QT import FormLayout, Iconos, QTMessages, WindowJuicio
...
class ManagerFideFics(Manager.Manager):
    ...
    def elige_juego(self, nivel):
        ...
        dbf = db.dbfT(
            "data",
            "ROWID",
            condicion="LEVEL=%d AND WHITE=%d" % (nivel, 1 if color else 0),
        )
        dbf.leer()
    ...
    def base_inicio(self, id_game):
        ...
        self.manager_analyzer = self.procesador.analyzer_clone(self.min_mstime, 0, multipv)
    ...
    def configurar_local(self):
        li_extra_options = [
            (
                "adjudicator_options",
                _("Adjudicator") + " - " + _("Options"),
                Iconos.Engines(),
            ),
        ]

        resp = Manager.configurar(self, li_extra_options)
        if resp:
    ...
    def change_adjudicator_options(self):
        ...
        form = FormLayout.FormLayout(
            self.main_window,
            _("Adjudicator") + " - " + _("Options") + " - " + self._TIPO,
            Iconos.Engines(),
            with_default=True,
        )
        form.separador()
        ...
        fich_log = self.manager_analyzer.fichero_log
        self.manager_analyzer.terminar()
        self.manager_analyzer.set_multipv_var(multipv)
        self.manager_analyzer.options(self.min_mstime, 0, True)
        if fich_log:
            self.manager_analyzer.log_open()
        self.play_next_move()
    ...
    def rendirse(self):
        if self.state == ST_ENDGAME:
            ...
            return True
        if len(self.game) > 0:
            if not QTMessages.pregunta(self.main_window, _("Do you want to resign?") + " (%d)" % self.plost):
    ...
    def analyze_begin(self):
        if not self.is_finished():
            if self.continueTt:
                self.manager_analyzer.ac_inicio(self.game)
            else:
                self.manager_analyzer.ac_inicio_limit(self.game)
            self.is_analyzing = True

    def analyze_state(self):
        self.manager_analyzer.engine.ac_lee()
        self.mrm = copy.deepcopy(self.manager_analyzer.ac_estado())
        return self.mrm

    def analyze_minimum(self):
        self.mrm = copy.deepcopy(self.manager_analyzer.ac_minimo(self.min_mstime, False))
        return self.mrm

    def analyze_end(self):
        if self.is_analyzing:
            self.is_analyzing = False
            self.manager_analyzer.ac_final(-1)
        ...
        if self.book:
            ...
            if si_book_usu and si_book_obj:
                if jg_obj.movimiento() != jg_usu.movimiento():
                    ...
                    QTMessages.message_information(self.main_window, comment.replace("\n", "<br>"))
        ...
        if si_analiza_juez:
            with QTMessages.analizando(self.main_window):
                ...
                if rm_usu is None:
                    self.analyze_end()
                    continue_tt = False
                    rm_usu = self.manager_analyzer.valora(position, jg_usu.from_sq, jg_usu.to_sq, jg_usu.promotion)
                    mrm.add_rm(rm_usu)
                ...
                if rm_obj is None:
                    self.analyze_end()
                    continue_tt = False
                    rm_obj = self.manager_analyzer.valora(position, jg_obj.from_sq, jg_obj.to_sq, jg_obj.promotion)
            ...
            w = WindowJuicio.WJuicio(
                ...
                rm_usu,
                analysis,
                is_competitive=not self.show_all_evaluations,
