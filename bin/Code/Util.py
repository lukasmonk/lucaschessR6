...
def temporary_file(path_temp: str, ext: str) -> str:
    ...
    while True:
        fich = opj(path_temp, "%d.%s" % (random.randint(1, sys.maxsize), ext))
...
def save_obj_pickle(obj, li_exclude: Optional[list] = None):
    ...
    try:
        ...
        if is_windows():
            ...
            env['DBUS_SESSION_BUS_ADDRESS'] = os.getenv(
                'DBUS_SESSION_BUS_ADDRESS', 'unix:path=/run/user/{os.getuid()}/bus'
            )
