...
class Arbiter:
    ...
    def __init__(self, owner, wowner, name_obj: str, rut_player_move: Callable):
        ...
        self.nom_engine, self.multipv, self.ms_time, self.show_all = self.get_arbiter_options()
        ...
        self.state: ArbiterState = ArbiterState.active
    ...
    def analyze_begin(self, game: Game.Game):
        self.game = game
        self.state = ArbiterState.analysing
        self.mrm = None
        self.move_played = None
        self.obj_move = None
        self.user_move = None
    ...
    def is_analysing(self):
        return self.state == ArbiterState.analysing
    ...
    def close(self):
        if self.manager_analyzer is not None:
            if self.is_analysing():
                ...
                self.state = ArbiterState.closing
                self.manager_analyzer.stop()
    ...
    def bestmove_found(self, _bestmove):
        ...
        if self.state == ArbiterState.analysing:
        ...
        if self.state == ArbiterState.closing:
            self.close()
    ...
    def get_arbiter_options(self) -> tuple:
    ...
    def change_arbiter_options(self):
        ...
        form = FormLayout.FormLayout(
            self.wowner, f"{_('Arbiter')} - {_('Options')}", Iconos.Engines(), with_default=True
        )
        form.separador()
