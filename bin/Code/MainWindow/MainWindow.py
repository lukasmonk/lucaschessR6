...
class MainWindow(LCDialog.LCDialog):
    ...
    def __init__(self, manager, owner=None, extparam=None):
        ...
        self.cursorthinking = QCursor(
            Iconos.pmThinking() if self.manager.configuration.x_cursor_thinking else Qt.CursorShape.BlankCursor
        )
        self.cursorthinking_rival = QCursor(Iconos.pmConnected())
        self.onTop = False
        ...
        ctrl1 = QShortcut(self)
        ctrl1.setKey(QKeySequence("Ctrl+1"))
        ctrl1.activated.connect(self.pressed_shortcut_Ctrl1)

        ctrl2 = QShortcut(self)
        ctrl2.setKey(QKeySequence("Ctrl+2"))
        ctrl2.activated.connect(self.pressed_shortcut_Ctrl2)

        alt_a = QShortcut(self)
        alt_a.setKey(QKeySequence("Alt+a"))
        alt_a.activated.connect(self.pressed_shortcut_alt_a)

        ctrl_f10 = QShortcut(self)
        ctrl_f10.setKey(QKeySequence("Ctrl+0"))
        ctrl_f10.activated.connect(self.pressed_shortcut_Ctrl0)

        F11 = QShortcut(self)
        F11.setKey(QKeySequence("F11"))
    ...
    def pressed_shortcut_F12(self):
        if not self.trayIcon:
            restore_action = QAction(Iconos.PGN(), _("Show"), self, triggered=self.restauraTrayIcon)
            quit_action = QAction(Iconos.Terminar(), _("Quit"), self, triggered=self.quitTrayIcon)
    ...
    def muestra(self):
        flags = Qt.WindowType.Dialog if self.owner else Qt.WindowType.Widget
        flags |= Qt.WindowType.WindowTitleHint | Qt.WindowType.WindowMinimizeButtonHint | Qt.WindowType.WindowMaximizeButtonHint
        if self.onTop:
            flags |= Qt.WindowType.WindowStaysOnTopHint

        self.setWindowFlags(Qt.WindowType.WindowCloseButtonHint | flags)
    ...
    def thinking(self, si_pensando):
        if si_pensando:
            if not self.cursor_pensando:
                QApplication.setOverrideCursor(self.cursorthinking_rival)
        else:
    ...
    def pensando_tutor(self, si_pensando):
        if si_pensando:
            if not self.cursor_pensando:
                QApplication.setOverrideCursor(self.cursorthinking)
        else:
            if self.cursor_pensando:
                QApplication.restoreOverrideCursor()
        self.cursor_pensando = si_pensando
    ...
    def check_translated_help_mode(self):
        ...
        QTimer.singleShot(3000, self.check_translated_received)
    ...
    def check_translated_received(self):
        ...
        if self.work_translate.check_commits():
            QTimer.singleShot(salto, self.check_translated_received)
    ...
    def deactivate_eboard(self, ms=500):
        if Code.eboard and Code.eboard.driver:
            ...
            if ms > 0:
                QTimer.singleShot(ms, deactive)
            else:
                deactive()

    @staticmethod
    def delay_routine(ms, routine):
        QTimer.singleShot(ms, routine)
