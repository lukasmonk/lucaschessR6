...
class WBase(QtWidgets.QWidget):
    ...
    def pon_toolbar(self, li_acciones, separator=False, with_shortcuts=False, with_eboard=False):
        ...
        for n, k in enumerate(li_acciones):
            ...
            if k not in (TB_NEXT, TB_PREVIOUS) and self.tb.toolButtonStyle() != QtCore.Qt.ToolButtonStyle.ToolButtonIconOnly:
        ...
        if self.with_shortcuts:
            if 49 <= k <= 57:
                ...
                if (m & QtCore.Qt.KeyboardModifier.AltModifier.value) > 0:
                    ...
                    return
        self.key_pressed("V", event.key(), event.modifiers().value)
