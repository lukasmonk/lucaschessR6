...
from Code.QT import Iconos, QTDialogs, QTMessages, QTUtils, SelectFiles, WindowPgnTags
...
class ManagerSolo(Manager.Manager):
    ...
    def save_file(self, file):
        ...
        if Util.save_pickle(file, dic):
            ...
            QTMessages.message_error(self.main_window, "%s : %s" % (_("Unable to save"), file))
            return False
    ...
    def change_rival(self):
        ...
        if dic:
            ...
            self.manager_rival = self.procesador.create_manager_engine(rival, r_t, r_p, ADJUST_BETTER)
            if nodes:
                self.manager_rival.set_nodes(nodes)

            dic["ROTULO1"] = _("Opponent") + ": <b>" + self.manager_rival.engine.name
