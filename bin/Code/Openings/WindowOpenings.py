...
from Code import Util, Variations
...
class WOpenings(LCDialog.LCDialog):
    def __init__(self, owner, opening_block: Optional[OpeningsStd.Opening]):
        ...
        lyBM, tbBM = QTDialogs.ly_mini_buttons(self, "", siLibre=False, icon_size=24)
        self.tbBM = tbBM
    ...
    def ponActivas(self):
        ...
        if self.game.opening:
            txt = '%s<br><span style="color:gray;">%s</span>' % (
                txt,
                self.game.opening.tr_name,
            )
...
class OpeningsCustom(LCDialog.LCDialog):
    def __init__(self, procesador, owner=None):
        ...
        self.ficheroDatos = Code.configuration.paths.file_pers_openings()
