...
class WManualSave(LCDialog.LCDialog):
    ...
    def fns_select(self):
        dir_inicial = self.configuration.paths.folder_personal_trainings() if self.fns is None else os.path.dirname(self.fns)
    ...
    def savesolucion(self):
        ...
        def write_file(fich, f, txt, quien):
            ...
            try:
                f.write(txt)
                QTMessages.temporary_message(self, "%s: %s" % (quien, _("Saved")), time)
            except:
