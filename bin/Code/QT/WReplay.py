...
from Code.QT import FormLayout, Iconos, QTUtils
...
def param_replay(configuration, parent, with_previous_next) -> bool:
    ...
    if with_previous_next:
        form.checkbox(
            _("Replay of the current and following games"),
            dic_var.get("REPLAY_CONTINUOUS", False),
        )
        form.separador()
...
class Replay:
    ...
    def process_toolbar(self, key):
        if key == TB_END_REPLAY:
            ...
            self.pausa()
            if param_replay(Code.configuration, self.main_window, False):
                dic_var = read_params()
                self.seconds = dic_var["SECONDS"]
                self.if_start = dic_var["START"]
                self.with_pgn = dic_var["PGN"]
